...
#
# v1.15.0 - tout le corps vit dans main(), appele en avant-derniere ligne :
# un telechargement tronque (curl | bash coupe en plein transfert) laisse
# une fonction incomplete que bash refuse de parser -- RIEN ne s'execute.
# Au niveau module il n'y a que des definitions, jamais d'effet de bord.
# La toute derniere ligne est un marqueur de fin que le wrapper
# landing/install.sh verifie AVANT d'executer le fichier telecharge.
...
main() {
    ...
    # Nettoyage d'un .app existant cote Bureau (souvent synchronise iCloud,
    # source des problemes 'L'application n'est plus ouverte').
